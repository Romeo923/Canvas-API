...
def applyCommand(commands, kwargs):
    match [list(command) if command[0] == '-' else command for command in commands]:
        ...
        case [['-', *flags], *args]:
            ...
            data |= kwargs
            left_over = [flag for flag in flags if flag not in 'urDiafg']
...
def generateAction(flags, name, ext):
    ...
    cflags = {flag: True if flag in flags else False for flag in "urDiafsg"}
    ...
    match cflags:
        ...
        #     input_data.append(assignment_id)

        #     def temp(*input_data, secondary_data):
        #         submission_url = canvasAPI.getSubmissions(*input_data)['html_url']
